...
##THIS FILE DEMONSTRATES THE FAILURE OF THE FIRST ALGORITHM WITH THE NEW GRID
##WEIGHTED CELLS ARE DENOTED BY "+"
...
#Retrieving Grid data from function that creates the grid
    ...
    return prev, start, goal, size, obstacles, isFound,weightedCells

prev, start, goal,size, obstacles, isFound, weightedCells = findPath()
    ...
    for x in range(w):
        for y in range(h):
            ...
            if index in weightedCells:
                el="+ "                 #Print weighted elements on grid
